...
export default function Home() {
  ...
  const filledCount = Math.min(totalTarget, plannedMeals.length);
  ...
  const selectedEstimate = useMemo(() => Math.round(plannedMeals.reduce((sum, meal) => sum + parseServingCost(meal) * people, 0) * .88), [plannedMeals, people]);
  const storeEstimates = useMemo(() => [
    { name: "Whole Foods", price: Math.round(selectedEstimate * 1.08), availability: 96 },
    { name: "Jewel-Osco", price: Math.round(selectedEstimate * .98), availability: 98 },
    { name: "Trader Joe’s", price: Math.round(selectedEstimate * .92), availability: 86 },
  ], [selectedEstimate]);
  const groceryGroups = useMemo(() => {
    ...
    plannedMeals.flatMap((meal) => meal.ingredients || []).forEach((ingredient) => {
      ...
      seen.add(key);
      const haystack = `${ingredient.aisle || ""} ${name}`.toLowerCase();
      const group = /produce|vegetable|fruit|herb|onion|garlic|lettuce|spinach|tomato|pepper|lemon|lime/.test(haystack) ? "Produce"
        : /meat|seafood|fish|salmon|shrimp|beef|pork|chicken|turkey|tuna|cod/.test(haystack) ? "Meat & seafood"
          : /milk|cheese|yogurt|refrigerated|egg|butter/.test(haystack) ? "Refrigerated"
            : /bakery|bread|tortilla|pita|bun/.test(haystack) ? "Bakery" : "Pantry";
  ...
  function mapRecipe(recipe: Record<string, unknown>, index: number, kind = "Dinner"): Meal {
    ...
    const tags = [
      ...
      ...(kind === "School lunch" ? ["Kid-friendly", "Packable"] : []),
    ...
    return {
      ...
      pricePerServing: Number(recipe.pricePerServing || 420) / 100,
      ingredients,
      tags: [...new Set(tags)],
    };
  }
  ...
  async function generatePlan(queryOverride?: string) {
    ...
    try {
      ...
      setRecipeNotice("Recipes are temporarily unavailable. Please try again.");
  ...
  async function findSimilar(meal: Meal) {
    ...
    try {
      const response = await fetch(`/api/recipes/search?${new URLSearchParams({
        q: meal.title, maxTime: String(meal.readyMinutes || 45), glutenFree: String(glutenFree), lowDairy: String(lowDairy),
  ...
  return <main>
    ...
    {view === "plan" && <div className="shell">
      ...
      <section className="planner">
        ...
        <button className="primary" onClick={() => generatePlan()} disabled={planning}>{planning ? "Building your recipe catalog…" : "Browse recipes for my plan"} <span>→</span></button><p className="estimate">Estimated groceries for a full plan: <strong>${planningEstimate.low}–${planningEstimate.high}</strong>{planningEstimate.high > budget && <span> · above your ${budget >= 500 ? "$500+" : `$${budget}`} target</span>}</p>
      </section>
    </div>}
    ...
    {view === "meals" && <div className="dashboard catalog-dashboard">
      ...
      <section className="plan-progress" aria-label={`${filledCount} of ${totalTarget} meal slots filled`}>
        <div className="progress-copy"><span>{filledCount} / {totalTarget}</span><div><strong>{planIsFull ? "Your schedule is full" : `${totalTarget - filledCount} meal slots left`}</strong><small>{range} plan · estimated groceries ${selectedEstimate ? `$${selectedEstimate}–$${Math.round(selectedEstimate * 1.12)}` : "$0"}</small></div></div>
      ...
      <section className="recipe-library">
        ...
        <div className="preference-filter-row" aria-label="Active meal preferences">
          ...
          <button className={kidLunches ? "active" : ""} aria-pressed={kidLunches} onClick={() => setKidLunches(!kidLunches)}>School lunches</button>
      ...
      <section className="selection-board">
        <div className="selection-heading"><div><p className="eyebrow">YOUR SCHEDULE</p><h3>Selected meals</h3><span>Collapse any section to keep a long week or month easy to scan.</span></div>{plannedMeals.length > 0 && <button className="outline" onClick={() => setPlannedMeals([])}>Clear selections</button>}</div>
        {activeMealKinds.map((kind) => {
          const selected = plannedMeals.filter((meal) => meal.kind === kind);
          const target = mealTargets[kind as keyof typeof mealTargets];
          return <details className="selected-meal-section" open key={kind}><summary><span>{kind === "School lunch" ? "🍱" : kind === "Lunch" ? "🥗" : "🍽"} {kind}s</span><small>{selected.length} of {target} selected</small></summary>{selected.length ? <div className="selected-meal-list">{selected.map((meal) => <article key={meal.id}><span>{meal.day}<b>{meal.date}</b></span><div><strong>{meal.title}</strong><small>{meal.sourceName} · {meal.time} · {meal.cost}</small></div><button onClick={() => removePlannedMeal(meal.id)} aria-label={`Remove ${meal.title} from ${kind}`}>Remove</button></article>)}</div> : <p className="empty-selection">Choose {target} {kind.toLowerCase()} recipe{target === 1 ? "" : "s"} from the catalog above.</p>}</details>;
        })}
      </section>
    ...
    {view === "list" && <div className="dashboard"><div className="page-heading"><div><p className="eyebrow">GROCERIES · {plannedMeals.length} SELECTED MEALS</p><h2>Everything you need, sorted.</h2><p>{groceryGroups.reduce((sum, group) => sum + group.count, 0)} unique ingredients for {people} people near {location}. Estimated total: ${selectedEstimate}–${Math.round(selectedEstimate * 1.12)}.</p></div><button className="outline" onClick={() => setView("meals")}>← Back to recipes</button></div><div className="list-layout"><section className="grocery-panel"><div className="store-compare"><span className="mini-label">COMPARE NEARBY STORES</span><div>{storeEstimates.map((store) => <button key={store.name} className={selectedStore === store.name ? "selected-store" : ""} onClick={() => setSelectedStore(store.name)}><strong>{store.name}</strong><span>${store.price}</span><small>{store.availability}% estimated availability</small></button>)}</div></div><div className="grocery-head"><strong>{selectedStore}</strong><span>Recipe-derived estimate · prices vary by location</span></div>{groceryGroups.length ? groceryGroups.map((group) => <details open key={group.title}><summary><span>{group.icon} {group.title}</span><small>{group.count} items</small></summary><div className="checklist">{group.items.map((item) => <label key={item}><input type="checkbox" /> <span>{item}</span><em>for {people}</em></label>)}</div></details>) : <p className="empty-state">Select recipes to build your grocery list.</p>}</section><aside className="export-panel"><span className="mini-label">READY WHEN YOU ARE</span><h3>Take your plan with you</h3><p>Send lists, recipes, and reminders where you already use them.</p><button onClick={copyForReminders}><span className="icon-centered">✓</span><div><strong>Apple Reminders</strong><small>Copy this grocery list</small></div><b>Copy</b></button><button onClick={downloadCalendar}><span className="icon-centered">31</span><div><strong>Google or Apple Calendar</strong><small>Import daily recipes</small></div><b>Export</b></button><div className="email-export"><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" /><button onClick={emailRecipes}><span className="icon-centered">@</span><div><strong>Email me recipes</strong><small>Send the complete plan</small></div><b>Email</b></button></div>{exportStatus && <p className="export-status">{exportStatus}</p>}</aside></div></div>}
